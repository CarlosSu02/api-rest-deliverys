...
class CategoriesServices {
    ...
    public searchCategoryById = async (id: number) => {
        ...
        if(category === null) throw new Error(JSON.stringify({ code: 404, message: 'Category is not exists! The following categories exist...', results: (await this.getCategories(authController.token.role)).results  }));
